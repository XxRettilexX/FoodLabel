      ...
      <SurfaceCard>
        ...
        {production.output_quantity ? (
          <Text style={styles.meta}>
            Output: {production.output_quantity} {production.output_unit || ''}
          </Text>
        ) : null}
      </SurfaceCard>

      <AppButton
        label="Vista origine ingredienti"
        onPress={() => navigation.navigate('IngredientOrigin', { productionId: production.id })}
      />
        ...
        production.inputs.map((input) => (
          <SurfaceCard key={input.id} style={styles.inputCard}>
            ...
            <Text style={styles.inputMeta}>
              Lotto {input.lot?.batch_number || input.lot_id} • {input.quantity_used} {input.unit}
            </Text>
          </SurfaceCard>
        ))
      ) : (
        <SurfaceCard>
          <Text style={styles.inputMeta}>Nessun input disponibile.</Text>
        </SurfaceCard>
      )}
    </ScrollView>
  );
