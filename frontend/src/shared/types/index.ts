...
export interface Lot extends Timestamps {
  ...
  labels?: Label[];
  // BUG 12 FIX: Renamed from `Alert` to `LotAlert` to avoid collision with React Native's Alert module.
  alerts?: LotAlert[];
}
...
// ── LotAlert (riferimento leggero) ───────────────────
// BUG 12 FIX: Renamed from `Alert` to `LotAlert` to avoid name collision
// with React Native's built-in Alert module.

export interface LotAlert extends Timestamps {
  id: number;
  lot_id: number;
  type: string;
  message: string;
